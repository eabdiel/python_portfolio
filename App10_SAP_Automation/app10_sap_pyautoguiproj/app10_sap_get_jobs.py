    ...
    for job in data:
        if job[2] == 'Canceled':
            ...
            soup = bs4.BeautifulSoup(
                open(f'{filepath}/Job_Reports/Error_Logs/{filenamedate}/{job_name}_{filenamedate}.html'), 'html.parser')
            table = soup.find('table', attrs={'class': 'list'})
            table_body = table.find('tbody')
            rows = table_body.find_all('tr')
            for row in rows:
            ...
            # data here has the values from the table in the html
            # save this to csv for easier data analysis later
            with open(f'{filepath}/Job_Reports/Error_Logs/{filenamedate}/{job_name}_{filenamedate}.txt', 'w',
                      newline='') as fou:
                cw = csv.writer(fou)
                cw.writerows(data)
        else:
