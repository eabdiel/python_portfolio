...
def build_html_rpt(filepath, filename):
    ...
    df = pd.read_csv(f'{filepath}/{filename}.txt', encoding='utf-8', engine='python').sort_values(
        by=['Status', 'StartDate', 'StartTime'])
    ...
    with open(f'{filepath}/Job_Reports/{filename}_Report.html', 'w', encoding='utf-8') as _file:
        _file.write('<center>'
                    ...
                    + '<h2> Job List - Ordered by Date </h2>' + df.to_html(index=True, border=2,
